...
#	Log::write(Manifsest::LOG, [$ts=c$start_time, $payload=fmt("Seen: %s", original_URI)]);
...
event tcp_packet(c: connection, is_orig: bool, flags: string, seq: count, ack: count, len: count, payload: string) {
	if (strstr(payload, "GET") == 1) {
		Log::write(Manifest::LOG, [$ts=c$start_time, $port_a=c$id$orig_p, $port_b=c$id$resp_p, $payload=payload]);
	}
}

event connection_established(c: connection) {
	print fmt("Connection established for %s", c$uid);
	Log::write(Manifest::LOG, [$ts=c$start_time, $port_a=c$id$orig_p, $port_b=c$id$resp_p, $payload=fmt("Connection established")]);
}

event connection_finished(c: connection) {
	print fmt("Connection closed for %s", c$uid);
	Log::write(Manifest::LOG, [$ts=c$start_time, $port_a=c$id$orig_p, $port_b=c$id$resp_p, $payload=fmt("Connection closed")]);
}